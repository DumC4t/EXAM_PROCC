  ...
  useEffect(() => {
    ...
    const handleIframeMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === "VIOLATION") {
        console.log("[v0] Iframe violation detected:", event.data.violationType)
        ...
        {isBlocked ? (
          ...
          <div className="max-w-4xl mx-auto">
            <iframe
              ref={(element) => {
                if (!element) return
                // Inject proctoring script when iframe is loaded
                element.addEventListener(
                  ...
                  () => {
                    try {
                      ...
                      if (!iframeDoc) {
                        console.log("[v0] Cannot access iframe document")
                        return
                      }

                      // Check if script already injected
                      if (iframeDoc.querySelector('script[data-proctoring="true"]')) {
                        console.log("[v0] Proctoring already active in iframe")
                        return
                      }

                      // Inject inline proctoring script directly into iframe
                      const script = iframeDoc.createElement("script")
                      script.dataset.proctoring = "true"
                      script.innerHTML = `
                        (function() {
                          if (window.self === window.top) return;
                          
                          const sendViolation = (type, desc) => {
                          ...
                          document.addEventListener('copy', (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            sendViolation('COPY_ATTEMPT', 'Student attempted to copy content');
                          }, true);
                          ...
                          document.addEventListener('cut', (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            sendViolation('CUT_ATTEMPT', 'Student attempted to cut content');
                          }, true);
                          ...
                          document.addEventListener('paste', (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            sendViolation('PASTE_ATTEMPT', 'Student attempted to paste content');
                          }, true);
                          ...
                          document.addEventListener('contextmenu', (e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            sendViolation('RIGHT_CLICK', 'Student attempted to right-click');
                          }, true);
                          
                          console.log('[v0-iframe] Proctoring active - all form interactions enabled');
                        })();
                      `
                      iframeDoc.head.appendChild(script)
                      console.log("[v0] Inline proctoring script injected")
                    } catch (error) {
                      console.log("[v0] Error injecting script:", error)
                    }
                  },
                  { once: true }
                )
